...
Updates available. v%s -> %s
  ...
  run 'dma update' to get the latest.
...
func CheckUpdate(manual bool) (ver ProgramVersion, err error) {
	...
	checkURL := "https://api.github.com/repos/hofstadter-io/dma/releases/latest"
	if UpdateVersionFlag != "" {
		checkURL = "https://api.github.com/repos/hofstadter-io/dma/releases/tags/" + UpdateVersionFlag
		manual = true
	}
...
func WaitPrintUpdateAvailable() {
	if os.Getenv("DMA_UPDATES_DISABLED") != "" {
		return
	}
